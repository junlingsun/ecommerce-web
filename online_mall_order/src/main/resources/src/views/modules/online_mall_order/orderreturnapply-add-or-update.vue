    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          status: [
            { required: true, message: '????״̬[0->????????1->?˻??У?2->?????ɣ?3->?Ѿܾ不能为空', trigger: 'blur' }
          ],
          handleTime: [
            { required: true, message: '????ʱ?不能为空', trigger: 'blur' }
          ],
          ...
          receiveMan: [
            { required: true, message: '?ջ不能为空', trigger: 'blur' }
          ],
          receiveTime: [
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/online_mall_order/orderreturnapply/info/${this.dataForm.id}`),
              method: 'get',
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/online_mall_order/orderreturnapply/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
