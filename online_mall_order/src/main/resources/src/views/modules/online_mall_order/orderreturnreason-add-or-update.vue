<template>
  <el-dialog
    ...
    <el-form-item label="?˻?ԭ?" prop="name">
      <el-input v-model="dataForm.name" placeholder="?˻?ԭ?"></el-input>
    </el-form-item>
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/online_mall_order/orderreturnreason/info/${this.dataForm.id}`),
              method: 'get',
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/online_mall_order/orderreturnreason/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
