    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          totalAmount: [
            { required: true, message: '֧???ܽ不能为空', trigger: 'blur' }
          ],
          subject: [
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/online_mall_order/paymentinfo/info/${this.dataForm.id}`),
              method: 'get',
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/online_mall_order/paymentinfo/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
